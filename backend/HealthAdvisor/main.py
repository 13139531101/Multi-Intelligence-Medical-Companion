...
def main(host, port, agent_prompt_file, model_name, provider, mcp_config_path, agent_url=""):
    ...
    try:
        ...
        skill = AgentSkill(
                id="HealthAdvisor",
                name="健康顾问",
                description="提供专业的健康咨询、症状分析和疾病知识查询服务",
                tags=["health", "diagnosis", "consultation", "medical"],
                examples=[
                    "我最近头痛和发热，可能是什么原因？",
                    "请帮我分析一下高血压的症状和治疗方法",
                    "给我一些关于健康饮食的建议"
        ...
        try:
            ...
            memory_initialized = loop.run_until_complete(health_advisor_memory_service.initialize())
