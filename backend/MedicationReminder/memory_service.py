...
class MedicationReminderMemoryService:
    ...
    async def store_medication_record(self,
        ...
        try:
            ...
            # 构建完整的记忆内容结构
            memory_content = {
                "text": f"用药记录: {medication_name} {dosage} {frequency}",
    ...
    async def store_adherence_record(self,
        ...
        try:
            ...
            # 构建完整的记忆内容结构
            memory_content = {
                "text": f"依从性记录: {medication_name} {adherence_rate}",
    ...
    async def store_appointment_record(self,
        ...
        try:
            ...
            # 构建完整的记忆内容结构
            memory_content = {
                "text": f"复诊预约: {appointment_date} {doctor_name}",
    ...
    async def cleanup_memories(self, user_id: str = None, older_than_days: int = 365) -> int:
        ...
        try:
            ...
            # user_id 和 older_than_days 参数在这里可能无法完全按预期工作，除非我们使用 filters
            # 但 cleanup_expired_memories 不接受 filters
    ...
    async def _analyze_adherence_patterns(self, user_id: str, time_period_days: int) -> List[Dict[str, Any]]:
        ...
        try:
            # 获取依从性记录
            adherence_memories = self.memory_system.search_memories(
    ...
    async def _check_drug_interactions(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            # 获取用户当前用药
            current_medications = self.memory_system.search_memories(
    ...
    async def _analyze_medication_patterns(self, user_id: str, time_period_days: int) -> List[Dict[str, Any]]:
        ...
        try:
            # 获取用药记录
            medication_memories = self.memory_system.search_memories(
    ...
    async def _generate_personalized_recommendations(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 获取用户档案
            profile_memories = self.memory_system.search_memories(
                query=f"user_id:{user_id} profile",
                agent_id="medication_reminder",
                user_id=user_id,
                memory_types=["medication_profile"],
                limit=5
            )
