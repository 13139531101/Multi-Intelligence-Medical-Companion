...
try:
    ...
    _db_config_spec = importlib.util.spec_from_file_location(
        "database_config", os.path.join(current_dir, "database_config.py")
    )
    assert _db_config_spec is not None
    _db_config_mod = importlib.util.module_from_spec(_db_config_spec)
    _db_config_spec.loader.exec_module(_db_config_mod)
    MemoryDatabaseConfig = _db_config_mod.MemoryDatabaseConfig

    _emb_service_spec = importlib.util.spec_from_file_location(
        "embedding_service", os.path.join(current_dir, "embedding_service.py")
    )
    assert _emb_service_spec is not None
    _emb_service_mod = importlib.util.module_from_spec(_emb_service_spec)
    _emb_service_spec.loader.exec_module(_emb_service_mod)
    EmbeddingService = _emb_service_mod.EmbeddingService
...
class MemoryRetrieval:
    ...
    def search_by_content(
        ...
        try:
            ...
            if not query_embedding:
                ...
                return self._search_by_text_match(
                    query, agent_id, user_id, memory_types, limit
                )
                ...
                if candidate["embedding_vector"]:
                    try:
                        ...
                        if similarity >= min_similarity:
                            ...
                            # 获取标签
                            candidate["tags"] = self._get_memory_tags(
                                cursor, candidate["memory_id"]
                            )
    ...
    def search_by_time_range(
        ...
        try:
            ...
            # 构建查询条件
            conditions = (
                "agent_id = %s AND user_id = %s AND created_at BETWEEN %s AND %s"
            )
            params: list[Any] = [agent_id, user_id, start_time, end_time]
    ...
    def search_by_tags(
        ...
        try:
            ...
            if match_all:
                ...
                tag_params: list[str | int] = tags + [len(tags)]
            else:
                tag_conditions = """
    ...
    def search_related_memories(
        ...
        try:
            ...
            cursor.execute(
                select_associations_sql, (memory_id, memory_id, memory_id, limit)
            )
            results = cursor.fetchall()
    ...
    def _search_by_text_match(
        ...
        try:
            ...
            params: list[Any] = [agent_id, user_id, f"%{query}%"]
