...
try:
    ...
    database_config_module = import_from_file("database_config", os.path.join(current_dir, "database_config.py"))
    MemoryDatabaseConfig = database_config_module.MemoryDatabaseConfig
    
    memory_storage_module = import_from_file("memory_storage", os.path.join(current_dir, "memory_storage.py"))
    MemoryStorage = memory_storage_module.MemoryStorage
    
    memory_retrieval_module = import_from_file("memory_retrieval", os.path.join(current_dir, "memory_retrieval.py"))
    MemoryRetrieval = memory_retrieval_module.MemoryRetrieval
    
    memory_manager_module = import_from_file("memory_manager", os.path.join(current_dir, "memory_manager.py"))
    MemoryManager = memory_manager_module.MemoryManager
    
    embedding_service_module = import_from_file("embedding_service", os.path.join(current_dir, "embedding_service.py"))
...
class AgentMemorySystem:
    ...
    async def initialize(self):
        ...
        """
        try:
            # 数据库已在__init__中初始化，这里可以添加其他异步初始化逻辑
