...
@mcp.tool()
def parse_medical_document(document_text: str, document_type: str = "auto") -> Dict[str, Any]:
    """
    解析医疗文档，提取关键信息
    ...
    try:
        ...
        if document_type == "auto":
            if "主诉" in document_text and "现病史" in document_text:
                ...
                document_type = "检验报告"
            elif "影像" in document_text or "CT" in document_text or "MRI" in document_text:
                document_type = "影像报告"
            else:
                document_type = "其他"
...
def _parse_visit_record(text: str) -> Dict[str, Any]:
    ...
    # 提取既往史
    past_history_match = re.search(r'既往史[：:]([^。]*(?:。[^体格检查诊断处理]*)*)', text)
    if past_history_match:
        info["past_history"] = past_history_match.group(1).strip()
    
    # 提取诊断
    diagnosis_matches = re.findall(r'诊断[：:]([^处理]+)', text)
    if diagnosis_matches:
        info["diagnosis"] = [d.strip() for d in diagnosis_matches[0].split('\n') if d.strip()]
...
def _parse_prescription(text: str) -> Dict[str, Any]:
    ...
    # 提取药物信息
    # 匹配格式：药名 剂量 用法 天数
    med_pattern = r'(\d+\.)?([^\d]+?)\s+(\d+(?:\.\d+)?\s*mg|\d+(?:\.\d+)?\s*g)\s+([^\d]+?)\s+(\d+)天'
    matches = re.findall(med_pattern, text)
    
    for match in matches:
        medication = {
            "name": match[1].strip(),
            "dosage": match[2].strip(),
            "usage": match[3].strip(),
            "duration": match[4] + "天"
        }
...
@mcp.tool()
def generate_visit_summary(documents: List[Dict[str, Any]], summary_type: str = "comprehensive") -> Dict[str, Any]:
    """
    生成就诊摘要
    ...
    try:
        ...
        for doc in documents:
            parsed = parse_medical_document(doc.get("content", ""), doc.get("type", "auto"))
            if parsed["success"]:
