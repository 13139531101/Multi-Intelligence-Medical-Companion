...
class TestMemorySystem(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 创建记忆系统实例
        with patch(
            "memory_system.MemoryDatabaseConfig", return_value=self.mock_db_config
        ):
    ...
    def test_search_memories(self, mock_search):
        """测试搜索记忆"""
        expected_results = [
            {"id": "memory_123", "content": self.test_content, "similarity": 0.85}
        ]
    ...
    def test_get_recent_memories(self, mock_recent):
        ...
        memories = self.memory_system.get_recent_memories(
            agent_id=self.test_agent_id, user_id=self.test_user_id, hours=24
        )
    ...
    def test_perform_maintenance(
        self, mock_compress, mock_cleanup_low, mock_cleanup_expired
    ):
        """测试执行系统维护"""
        ...
        results = self.memory_system.perform_maintenance(
            agent_id=self.test_agent_id, user_id=self.test_user_id
        )
...
class TestMemorySystemConfig(unittest.TestCase):
    ...
    def test_get_agent_config(self):
        ...
        self.assertEqual(
            unknown_config, MemorySystemConfig.AGENT_SPECIFIC_CONFIG["general"]
        )
    ...
    def test_get_memory_type_config(self):
        ...
        self.assertEqual(
            unknown_config, MemorySystemConfig.MEMORY_TYPES_CONFIG["working"]
        )
...
class TestEmbeddingService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch.dict(
            ...
            with patch(
                "embedding_service.SentenceTransformer", return_value=mock_model
            ):
    ...
    def test_calculate_similarity(self):
        ...
        # 相同向量的相似度应该为1
        similarity1 = self.embedding_service.calculate_similarity(
            embedding1, embedding2
        )
        self.assertAlmostEqual(similarity1, 1.0, places=5)

        # 正交向量的相似度应该为0
        similarity2 = self.embedding_service.calculate_similarity(
