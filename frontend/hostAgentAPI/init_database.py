...
def _build_db_dsn(cfg: dict) -> str:
...
def _get_pool(cfg: dict) -> ConnectionPool:
    global _db_pool
...
def init_database():
    ...
    try:
        ...
        with pool.connection() as connection:
            ...
            cursor.execute("ALTER TABLE user_medications ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE")
            cursor.execute("ALTER TABLE user_medications ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN DEFAULT FALSE")
            try:
                cursor.execute(
                    "ALTER TABLE user_medications ALTER COLUMN is_active TYPE BOOLEAN "
                    "USING (CAST(is_active AS TEXT) IN ('1','t','true','y','yes','on'))"
                )
            except Exception:
                pass
            try:
            ...
            cursor.execute("ALTER TABLE reminders ADD COLUMN IF NOT EXISTS is_completed BOOLEAN DEFAULT FALSE")
            cursor.execute("ALTER TABLE reminders ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ")
            cursor.execute("ALTER TABLE reminders ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN DEFAULT FALSE")
            try:
                cursor.execute(
            ...
            cursor.execute("ALTER TABLE medication_reminders ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE")
            cursor.execute("ALTER TABLE medication_reminders ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN DEFAULT FALSE")
            try:
                cursor.execute(
                    "ALTER TABLE medication_reminders ALTER COLUMN is_active TYPE BOOLEAN "
                    "USING (CAST(is_active AS TEXT) IN ('1','t','true','y','yes','on'))"
                )
            except Exception:
                pass
