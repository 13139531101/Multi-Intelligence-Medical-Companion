...
import { createConversation, sendMessage as sendMessageApi, queryEvents, getProcessingMessages } from '../api/api';
...
const AgentAssistant = ({ 
  ...
  const finalSuggestions = suggestions.length > 0 ? suggestions : currentAgent.suggestions;
    ...
    try {
      if (event?.content?.parts && Array.isArray(event.content.parts)) {
        text = event.content.parts
          .map((p) => (p.type === 'text' ? (p.text ?? '') : (typeof p.data === 'string' ? p.data : JSON.stringify(p.data))))
          .join('');
      }
    } catch {
      text = '';
      ...
      try {
        ...
        if (Array.isArray(events) && events.length > 0) {
          ...
          for (const ev of sorted) {
            if (ev.id && ev.content?.metadata?.conversation_id === convId && !processedEventIds.current.has(ev.id)) {
    ...
    <>
      ...
      >
        ...
        <Collapse in={expanded}>
          <DialogContent sx={{ p: 0, display: 'flex', flexDirection: 'column', height: sizeConfig.height - 120 }}>
            {/* 消息列表 */}
                  ...
                  {messages.map((message) => (
                    ...
                    >
                      ...
                      >
                        <Avatar
                          sx={{
                            bgcolor: message.sender === 'user' ? 'primary.main' : currentAgent.color,
                            width: 24,
                            height: 24
                          }}
                        >
