...
d = r2.json()
print("fields count:", len(d["parsed"]["fields"]))
print("sections count:", len(d["parsed"]["sections"]))
print("summary:", d["parsed"]["summary"])
for sec in d["parsed"]["sections"]:
    print(f"  - {sec['title']}: {len(sec['paras'])} paras, {len(sec['chips'])} chips")
    for p in sec['paras'][:1]:
        print(f"    para[:100]: {p[:100]}")
