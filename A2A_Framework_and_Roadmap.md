...
尽管框架已搭建完毕，但目前在“可视化”和“交互体验”上仍有缺失。以下是急需落地的功能：

### 2.1 交互可视化 (Interaction Visualization)
*   **现状**: Agent 之间的调用是“黑盒”的，只能在后台日志中看到。
*   **缺失**:
    *   **调用链路图**: 在 Admin Dashboard 中无法看到 A 调了 B，B 又调了 C 的全链路。
    *   **实时交互日志**: 前端缺少一个窗口来实时展示 Agent 之间的 JSON 消息往来。
*   **计划**: 在 Admin Dashboard 增加 "Trace View"（链路追踪）页面。

### 2.2 知识库管理 (RAG Management)
